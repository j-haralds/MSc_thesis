...
class BatteryECMM(nn.Module):
    ...
    def forward(self, I_batch, u_batch, soc0_batch, T_max):
        ...
        I_norm = I_batch / self.I_ref
        u_exp  = u_batch.squeeze(1)
...
def prepare_data(data, R0_func):
    ...
    for _, grp in data.sort_values(['trajectory', 't']).groupby('trajectory'):
        ...
        trajs.append(dict(
            I = torch.tensor(grp['I'].values,   dtype=torch.float32),  # sequence!
            u     = torch.tensor(grp['u'].values,   dtype=torch.float32),
            ...
            U1_true=torch.tensor(grp['Ue'].values - grp['I'].values * R0_func(grp['u'].values, grp['I'].values) - grp['V'].values, dtype=torch.float32),
        ))
    return trajs
...
@torch.no_grad()
def _predict_np(model, config, I_seq, u_seq, soc0, T, noise = False, noise_lvl = 0.00):
    # --- run model ---
    I_b    = I_seq.unsqueeze(0) if I_seq.ndim == 1 else I_seq
    #I_b    = torch.tensor([I_val],  dtype=torch.float32)
    u_b    = u_seq.unsqueeze(0) if u_seq.ndim == 1 else u_seq
    ...
    I_norm = torch.full((T,), I_b[3] / model.I_ref)
    u_t    = torch.full((T,), u_b[3])
...
def plot_noisy_inputs(trajs, noise_lvl = 0.00):
    ...
    ax[0].axvline(I_b[4], color=COLORS[0], ls='--', label='True I', lw=2)
    ax[0].legend()
    ax[0].set_xlabel('Current [A]')
    ax[0].set_ylabel('Frequency')
    ax[1].hist(u_plot.numpy() * 10, bins=20, color=COLORS[1], alpha=0.7, edgecolor='black')
    ax[1].axvline(u_b[4] * 10, color=COLORS[1], ls='--', label='True u', lw=2)
    ax[1].legend()
    ...
    if not time:
        for j in trange(n):
            ...
            for i, noise_lvl in enumerate(noise_lvls):
                ...
                rmse_noise[j,i] = np.sqrt(np.mean((V - tr['V'].cpu().numpy())**2))
